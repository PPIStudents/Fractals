...
namespace OOP_laba_8
{
    public partial class Form1 : Form
    {
        ...
        private void PressEventHandler(Object sender, KeyPressEventArgs e)
        {
            if (rep.getSize() != 0)
            {
                if (e.KeyChar == 119) rep.peek_cur().Move(0, -1);//w
                if (e.KeyChar == 115) rep.peek_cur().Move(0, 1);//s
                if (e.KeyChar == 97) rep.peek_cur().Move(-1, 0);//a
                if (e.KeyChar == 100) rep.peek_cur().Move(1, 0);//d
                if (e.KeyChar == 107) btnColor_Click(sender, e);//k
                if (e.KeyChar == 98) rep.peek_cur().Grow(1);//b
                if (e.KeyChar == 118) rep.peek_cur().Grow(-1);//v
                if (e.KeyChar == 110) rep.peek_cur().GrowN(2);//n
                if (e.KeyChar == 109) rep.peek_cur().GrowN(-2);//m
